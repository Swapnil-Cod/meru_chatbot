...
class ChatApp {
    ...
    async submitFeedback(feedbackType, question, sqlQuery) {
        try {
    ...
    showCorrectionModal(question, originalSql) {
        this.currentFeedbackQuestion = question;
        this.currentFeedbackSql = originalSql;

        const modal = document.getElementById('feedbackModal');
        const correctedSqlInput = document.getElementById('correctedSql');
        const explanationInput = document.getElementById('feedbackExplanation');

        correctedSqlInput.value = originalSql || '';
        explanationInput.value = '';

        modal.classList.add('active');
        correctedSqlInput.focus();
    }
    ...
    async submitCorrection() {
        const correctedSql = document.getElementById('correctedSql').value.trim();
        const explanation = document.getElementById('feedbackExplanation').value.trim();

        if (!correctedSql) {
            alert('Please provide the corrected SQL query');
            return;
        }
        ...
        try {
            const response = await fetch('/chat/feedback', {
                ...
                body: JSON.stringify({
                    user_question: this.currentFeedbackQuestion,
                    generated_sql: this.currentFeedbackSql,
            ...
            if (response.ok) {
                const data = await response.json();
                console.log('Correction submitted:', data.message);
